...
public class TratamentoDao extends ObjetoDao {
    ...
    public List<Tratamento> PesquisaMovTratamentos(int box, String pesq) throws SQLException {
        ...
        String sql = "select A.cd_freq, D.nm_freq , A.cd_trat, C.ds_tratamento, A.cd_mov, B.dt_inicial, B.dt_final, B.trat_dist, A.presenca "
                ...
                + " where data =  CURRENT_DATE and trat_dist = 0 and C.cd_tratamento != 6 and " + campos.get(box) + " ilike '" + pesq + "%' "
                + "or B.tp_periodo = 7 and trat_dist = 0 and C.cd_tratamento != 6 and " + campos.get(box) + " ilike '" + pesq + "%' order by D.nm_freq";
        PreparedStatement stmt = conexao.prepareStatement(sql);
        ResultSet rs = stmt.executeQuery();
    ...
    public List<Tratamento> PesquisaMovTratamentosComData(int box, String pesq, String data) throws SQLException, ParseException {
        List<String> campos = new ArrayList<String>();
        java.sql.Date dtBusca = new java.sql.Date(Utils.formatoDataSimples.parse(data).getTime());
        campos.add("D.nm_freq");
        campos.add("C.ds_tratamento");
        ...
        String sql = "select A.cd_freq, D.nm_freq , A.cd_trat, C.ds_tratamento, A.cd_mov, B.dt_inicial, B.dt_final, B.trat_dist, A.presenca "
                ...
                + "inner join tratamentos C on C.cd_tratamento = A.cd_trat "
                + "inner join frequentador D on D.cd_freq = A.cd_freq "
                + " where data =  ? and trat_dist = 0 and C.cd_tratamento != 6 and " + campos.get(box) + " ilike '" + pesq + "%' "
                + "or B.tp_periodo = 7 and trat_dist = 0 and C.cd_tratamento != 6 and " + campos.get(box) + " ilike '" + pesq + "%' order by D.nm_freq";
        PreparedStatement stmt = conexao.prepareStatement(sql);
        stmt.setDate(1, super.toSqlDate(dtBusca));
        ResultSet rs = stmt.executeQuery();
